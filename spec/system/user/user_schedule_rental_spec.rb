...
describe 'User schedules rental' do
  it 'successfully' do
  ...
  it 'and end date must be greater then start date' do
    subsidiary = create(:subsidiary, name: 'Almeida Motors')
    user = create(:user, subsidiary: subsidiary)
    manufacture = create(:manufacture)
    fuel_type = create(:fuel_type)
  ...
  it 'and cars of the chosen category must be available' do
    subsidiary = create(:subsidiary, name: 'Almeida Motors')
    user = create(:user, subsidiary: subsidiary)
    category = create(:category, name: 'A', daily_rate: 10)
    ...
    create(:car, car_model: car_model, status: :unavailable)
    create(:car, car_model: other_car_model, status: :available)
  ...
  it 'and price projection can not be zero or less' do
    subsidiary = create(:subsidiary, name: 'Almeida Motors')
    user = create(:user, subsidiary: subsidiary)
    manufacture = create(:manufacture)
    fuel_type = create(:fuel_type)
